...
import sovits_utils
from models import SynthesizerTrn_energy
...
class Svc(object):
    ...
    def load_model(self):
        # 获取模型配置
        self.net_g_ms = SynthesizerTrn_energy(
    ...
    def infer(self, speaker, tran, trans_energy, raw_path,
        ...
        with torch.no_grad():
            start = time.time()
            audio = self.net_g_ms.infer(c, f0=f0, g=sid, uv=uv, energy = energy, predict_f0=auto_predict_f0, noice_scale=noice_scale)[0,0].data.float()
